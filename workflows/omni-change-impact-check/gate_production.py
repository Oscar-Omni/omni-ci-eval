#!/usr/bin/env python3
"""Check an Omni model PR branch against production.

One of three interchangeable variants, see the workflows/omni-change-impact-check
README. branchPerPullRequest has already created a matching Omni branch, this
script resolves it, refreshes its schema against production, runs semantic
validation, then runs content validation against every dashboard labelled
verified.
...
def resolve_branch_id(cfg):
    ...
    sys.stderr.write(
        f"::error::No Omni branch named '{cfg.branch_name}' found. Available branches: {names}\n"
        "(confirm branchPerPullRequest is enabled on the model's git config)\n")
    sys.exit(1)
...
def main():
    ...
    print("[ok] Gate passed (production).")
