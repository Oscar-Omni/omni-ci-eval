...
A collection of GitHub Actions workflow templates for Omni repos. Each folder under `workflows/` is a self contained CI workflow with its own README, covering a different concern. None of it runs in this repo, it's a library to copy from into your own Omni or dbt repo.
...
* Impact checks, check a PR against Omni's schema and content validators so a model edit or an upstream dbt change can't silently break a dashboard you've labelled verified.
